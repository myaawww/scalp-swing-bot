...
# ── NEAR-MISS DIAGNOSTICS (added — mirrors Vectis's killer= logging so both
#    engines are equally debuggable; scan_symbol runs concurrently across an
#    8-worker pool, so the tally is lock-protected) ──────────────────────────
_near_miss_lock  = threading.Lock()
_near_miss_tally: Counter = Counter()
...
def _near_miss(symbol: str, combo: Combo, reason: str, **details) -> None:
    """Diagnostic logging for a rejected candidate — mirrors Vectis's
    `[NEAR MISS] ... killer=X` format so both engines are equally debuggable.
    Also tallies into _near_miss_tally so run_scan can print a per-scan
    breakdown of which gate is doing the rejecting most often."""
    with _near_miss_lock:
        _near_miss_tally[reason] += 1
    extra = " | ".join(f"{k}={v}" for k, v in details.items())
    suffix = f" | {extra}" if extra else ""
    print(f"  [NEAR MISS] {symbol} | combo={combo.label} | killer={reason}{suffix}")
...
def compute_signal(symbol: str, combo: Combo, btc_regime: str) -> NyxSignal | None:
    ...
    if atr_htf <= 0 or atr_exec <= 0:
        _near_miss(symbol, combo, "ATR_INVALID", atr_htf=atr_htf, atr_exec=atr_exec)
        return None
    ...
    if bias == "neutral":
        _near_miss(symbol, combo, "HTF_BIAS_NEUTRAL")
        return None
    ...
    if btc_regime_blocks(symbol, direction, btc_regime):
        _near_miss(symbol, combo, "BTC_REGIME_BLOCK", dir=direction, btc_regime=btc_regime)
        return None
    ...
    if bias == "bear" and pd["pct"] < combo.premium_threshold:
        _near_miss(symbol, combo, "PD_NOT_PREMIUM_SHORT", dir=direction,
                   pct=f"{pd['pct']:.2f}", threshold=combo.premium_threshold)
        return None
    ...
    if poi is None:
        _near_miss(symbol, combo, "NO_HTF_POI", dir=direction)
        return None
    ...
    if sweep is None:
        _near_miss(symbol, combo, "NO_EXEC_SWEEP", dir=direction)
        return None

    # ── Displacement + CHoCH (hard gate #2) ────────────────────────────────
    disp = detect_displacement(candles_exec, sweep, bias, atr_exec, swings_exec, combo)
    if disp is None:
        _near_miss(symbol, combo, "NO_DISPLACEMENT_CHOCH", dir=direction)
        return None
    ...
    if zone_hi <= zone_lo:
        _near_miss(symbol, combo, "INVALID_ENTRY_ZONE", dir=direction, zone_hi=zone_hi, zone_lo=zone_lo)
        return None
    ...
    if dist_atr > combo.entry_max_dist_atr:
        _near_miss(symbol, combo, "ENTRY_TOO_FAR", dir=direction,
                   dist_atr=f"{dist_atr:.2f}", max=combo.entry_max_dist_atr)
        return None
    ...
    if risk <= 0:
        _near_miss(symbol, combo, "ZERO_RISK", dir=direction)
        return None
    ...
    if abs(tp2 - exact_entry) / risk < combo.tp2_min_rr:
        _near_miss(symbol, combo, "TP2_RR_TOO_LOW", dir=direction,
    ...
    if score < min_score:
        _near_miss(symbol, combo, "CONFLUENCE_SCORE_TOO_LOW", dir=direction,
                   score=f"{score}/{min_score}", combos=combos_hit)
        return None
...
def run_scan() -> None:
    ...
    with _near_miss_lock:
        tally_snapshot = _near_miss_tally.most_common()
    if tally_snapshot:
        total_rejections = sum(n for _, n in tally_snapshot)
        print(f"  [NEAR MISS SUMMARY] {total_rejections} rejection(s) across "
              f"{len(pairs)} (symbol, pipeline) check(s) this scan:")
        for reason, count in tally_snapshot:
            print(f"    {reason:<28} {count:>3}  ({count / total_rejections * 100:.0f}%)")
